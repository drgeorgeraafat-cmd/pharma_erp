...
DOCTYPE_SPECS = {
    ...
    "Delivery Return Request": {
        ...
        "fields": [
            ...
            F("Request Source", "request_source", "Select", options="Driver Report\nShift Manager", default="Driver Report", read_only=1),
            F("Approved By", "approved_by", "Link", options="User", read_only=1),
            F("Approved At", "approved_at", "Datetime", read_only=1),
...
def _repair_stale_driver_return_flags():
    ...
    rows = frappe.get_all(
        ...
        filters={
            ...
            "custom_delivery_status": ["in", ["Out for Delivery", "Returning to Pharmacy"]],
        },
    ...
    for row in rows:
        expected = (
            "Returning to Pharmacy"
            if row.custom_delivery_status == "Returning to Pharmacy"
            else "Driver Out"
        )
        if str(row.custom_driver_return_status or "").strip() == expected:
            continue
        values = {"custom_driver_return_status": expected}
        if meta.has_field("custom_driver_returned_at"):
            values["custom_driver_returned_at"] = None
        frappe.db.set_value("Sales Invoice", row.name, values, update_modified=False)
        updated.append(row.name)
    return {"updated": len(updated), "invoices": updated}
...
def install():
    ...
    stale_driver_flags = _repair_stale_driver_return_flags()
    frappe.db.commit()
    frappe.clear_cache(doctype="Sales Invoice")
    result = verify()
    result.update({
        "field_results": field_results,
        "added_status_options": added_options,
        "stale_driver_flags": stale_driver_flags,
    })
    print("Delivery partial return workflow V2.22 installed successfully.")
    return result
...
def verify():
    ...
    request_meta = frappe.get_meta("Delivery Return Request") if not missing_doctypes and frappe.db.exists("DocType", "Delivery Return Request") else None
    required_request_fields = ["request_source", "approved_by", "approved_at"]
    missing_request_fields = [
        fieldname for fieldname in required_request_fields
        if not request_meta or not request_meta.has_field(fieldname)
    ]
    return {
        "version": VERSION,
        ...
        "ready": not missing_doctypes and not missing_fields and not missing_request_fields and not missing_options,
    }
